...
document.addEventListener('DOMContentLoaded', function() {
    // Get Bootstrap offcanvas elements
    const sidebarOffcanvas = document.getElementById('sidebarOffcanvas');
    const mobileNavOffcanvas = document.getElementById('mobileNavOffcanvas');
    const bsSidebar = new bootstrap.Offcanvas(sidebarOffcanvas);
    const bsMobileNav = new bootstrap.Offcanvas(mobileNavOffcanvas);

    // Handle window resize - close offcanvas on larger screens
    window.addEventListener('resize', function() {
        // Close offcanvas on larger screens if it's open
        if (window.innerWidth > 991) {
            if (sidebarOffcanvas.classList.contains('show')) {
                bsSidebar.hide();
            }
            if (mobileNavOffcanvas.classList.contains('show')) {
                bsMobileNav.hide();
            }
        }
    });
    ...
    dropdownItems.forEach(item => {
        ...
        // Keep dropdown functionality for keyboard navigation
    ...
    // Mobile navigation collapse functionality
    const mobileNavLinks = document.querySelectorAll('.mobile-nav-link[data-bs-toggle="collapse"]');
    ...
    mobileNavLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            mobileNavLinks.forEach(otherLink => {
                if (otherLink !== this) {
                    const otherTarget = document.querySelector(otherLink.getAttribute('data-bs-target'));
                    const otherChevron = otherLink.querySelector('.fa-chevron-right');
    ...
    const trackButton = document.querySelector('.btn-track-shipment');
    const getTouchButton = document.querySelector('.btn-get-in-touch');
    ...
    addLoadingState(trackButton);
    addLoadingState(getTouchButton);

    // Search functionality - Full width search bar
    ...
    searchButtons.forEach(button => {
        if (button.querySelector('.fa-search')) {
            button.addEventListener('click', function() {
                ...
                // Toggle full-width search input
                const existingSearch = document.querySelector('.search-overlay');
                if (existingSearch) {
                    existingSearch.remove();
                    document.querySelector('.main-navbar').classList.remove('search-active');
                } else {
                    // Create full-width search overlay
                    const searchOverlay = document.createElement('div');
                    searchOverlay.className = 'search-overlay';
                    searchOverlay.innerHTML = `
                        <div class="search-container">
                            <input type="text" class="search-input-full" placeholder="Search..." autocomplete="off">
                    ...
                    document.querySelector('.main-navbar').classList.add('search-active');

                    const searchInput = searchOverlay.querySelector('.search-input-full');
                    ...
                    document.addEventListener('keydown', function escapeSearch(e) {
                        if (e.key === 'Escape') {
                            closeSearch();
    ...
    searchButtons.forEach(button => {
        if (button.querySelector('.fa-user')) {
            button.addEventListener('click', function() {
                console.log('User menu triggered');
                // You can implement user menu/login modal here
                alert('User menu - Login/Register functionality would go here');
            });
        }
    });
    ...
    if (emailLink) {
        emailLink.addEventListener('click', function(e) {
            // The mailto: link will work automatically
            console.log('Email link clicked');
        });
    }

    // Phone number click functionality (for mobile)
    const phoneElements = document.querySelectorAll('.phone-number, .sidebar-section p');
    phoneElements.forEach(element => {
        const text = element.textContent;
        if (text && (text.includes('+') || text.includes('1-800'))) {
            element.style.cursor = 'pointer';
            element.addEventListener('click', function() {
                // Extract phone number and create tel: link
    ...
    console.log('LOGICO Navigation initialized successfully');
});
